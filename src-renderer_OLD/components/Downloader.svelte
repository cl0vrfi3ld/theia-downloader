...
<div class="align-center flex flex-col content-center items-center text-white">
	<form on:submit|preventDefault={doDownload} class="flex flex-col content-center items-center">
		<input
			type="text"
			bind:value={url}
			placeholder="youtube url"
			class="bg-action mb-[12px] h-8 rounded-2xl py-2 px-3 text-center text-white placeholder-white"
		/>
		<div class="mx-[54px] mb-[12px]">
			<TButton on:click={() => (format = 'mp3')} isActive={Boolean(format === 'mp3')}>mp3</TButton>
			<TButton on:click={() => (format = 'flac')} isActive={Boolean(format === 'flac')}
				>flac</TButton
			>
			<TButton
				on:click={() => (format = 'wav')}
		...
		<div class="mx-[54px] mb-[12px]">
			<TButton on:click={() => (fidelity = 'lowest')} isActive={Boolean(fidelity === 'lowest')}
				>lo-fi</TButton
			>
			<TButton on:click={() => (fidelity = 'medium')} isActive={Boolean(fidelity === 'medium')}
				>mid-fi</TButton
			>
			<TButton on:click={() => (fidelity = 'highest')} isActive={Boolean(fidelity === 'highest')}
				>hi-fi</TButton
			>
		</div>

		<TButton on:click={doDownload} {progress} wide disabled={dlbtnDisabled}>{status}</TButton>
		<br />
	</form>
	{#if isDev}
		<div>
			...
			>
			<TButton disabled={false} on:click={doDebugShow}>show update window</TButton>
		</div>
	{/if}
</div>
